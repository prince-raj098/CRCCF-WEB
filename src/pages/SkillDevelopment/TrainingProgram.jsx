...
import { programs } from '../../data/skillDevelopment/TrainingProgramData.js';
...
const Keyboard = () => {
  ...
  return (
    <div className="keyboard-base w-[85%] md:w-[70%] max-w-[650px] h-[90px] md:h-[120px] bg-[#d3cebe] rounded-lg border-b-[15px] border-r-[5px] border-[#a19c8f] shadow-[0_20px_40px_rgba(0,0,0,0.8),inset_0_2px_5px_rgba(255,255,255,0.8)] shrink-0" style={{ transform: 'perspective(800px) rotateX(25deg)' }}>
      ...
      <div className="w-full h-full p-2 md:p-4 flex gap-2 md:gap-4 justify-between">
         ...
         <div className="flex-1 flex flex-col justify-between h-full gap-1 md:gap-1.5">
           ...
           <div className="flex gap-1 md:gap-1.5 flex-1">
             {kbLayout[0].map((code) => <div key={code} className={getKeyClass(code, "flex-1")}></div>)}
           </div>
           {/* Row 2 */}
           <div className="flex gap-1 md:gap-1.5 flex-1 pl-2">
             {kbLayout[1].map((code, i) => <div key={code} className={getKeyClass(code, i === 13 ? "flex-[2]" : "flex-1")}></div>)}
           </div>
           {/* Row 3 */}
           <div className="flex gap-1 md:gap-1.5 flex-1 pl-4">
             {kbLayout[2].map((code, i) => <div key={code} className={getKeyClass(code, i === 0 ? "flex-[1.5]" : i === 13 ? "flex-[1.5]" : "flex-1")}></div>)}
           </div>
           {/* Row 4 */}
           <div className="flex gap-1 md:gap-1.5 flex-1 pl-6">
             {kbLayout[3].map((code, i) => <div key={code} className={getKeyClass(code, i === 0 ? "flex-[1.8]" : i === 12 ? "flex-[2.2]" : "flex-1")}></div>)}
           </div>
           {/* Row 5 */}
           <div className="flex gap-1 md:gap-1.5 flex-1 pl-2">
             {kbLayout[4].map((code, i) => <div key={code} className={getKeyClass(code, i === 0 ? "flex-[2.2]" : i === 11 ? "flex-[2.8]" : "flex-1")}></div>)}
           </div>
           {/* Row 6 (Spacebar row) */}
           <div className="flex space-x-1 md:space-x-1.5 flex-1 justify-center">
             {kbLayout[5].map((code) => <div key={code} className={getKeyClass(code, code === 'Space' ? "w-[40%]" : "w-[8%]")}></div>)}
           </div>
         </div>
...
const Mouse = () => {
  ...
  return (
    <div className="hidden sm:block w-12 md:w-16 h-16 md:h-20 bg-[#d3cebe] rounded-[2rem] rounded-t-[3rem] border-r-[6px] border-b-[10px] border-[#a19c8f] shadow-[0_15px_30px_rgba(0,0,0,0.8),inset_0_2px_5px_rgba(255,255,255,0.9)] relative shrink-0" 
         style={{ 
...
const ProgramCard = ({ program }) => {
  ...
  return (
    <div className={`border border-transparent hover:border-gray-300 flex flex-col items-center justify-start p-0.5 md:p-1 rounded-xl cursor-pointer ${program.color} bg-transparent hover:bg-black/5 transition-all duration-200 group`}>
      <div className="flex-shrink-0 flex items-center justify-center w-8 h-8 md:w-12 md:h-12 mb-0.5 md:mb-1 border-2 border-current bg-white/60 rounded-xl shadow-sm group-hover:shadow-md group-hover:bg-white group-hover:scale-110 transition-all">
        <Icon className="text-lg md:text-xl drop-shadow-sm" />
      </div>
      <div className="flex flex-col items-center justify-start w-full text-center">
        <span className="text-[9px] md:text-[10px] text-gray-800 uppercase font-bold leading-tight line-clamp-2 px-1">
          {program.name}
        </span>
      </div>
    </div>
    ...
    <div className="flex-1 flex flex-row p-3 md:p-5 pb-0 min-h-0 font-['VT323']">
      ...
      <div className="flex-1 flex flex-col h-full mr-2 md:mr-4 relative">
        ...
        <main 
          ...
          className="flex-1 overflow-y-auto custom-scrollbar relative z-20 pr-1 md:pr-2"
        >
          <div className="grid grid-cols-3 md:grid-cols-6 gap-x-0 md:gap-x-2 gap-y-0.5 md:gap-y-3 auto-rows-max pb-4 md:pb-8 pt-1 md:pt-2 px-0 md:px-2">
            {programs.filter(program => program.name.toLowerCase().includes(searchQuery.toLowerCase())).map((program) => (
  ...
  return (
    <div className="h-10 shrink-0 bg-[#c0c0c0] border-t border-gray-400 flex items-center justify-between px-2 text-black font-sans text-xs relative z-50">
      ...
      <div className="flex items-center space-x-1 h-full">
        ...
        <div className="hidden lg:flex items-center hover:bg-black/10 px-2 rounded-md h-8 cursor-pointer transition-colors mr-2">
          ...
          <div className="flex flex-col leading-tight">
            ...
            <span className="text-[10px] text-gray-700">Partly sunny</span>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center bg-white border-gray-400 border shadow-inner px-3 py-1.5 w-48 mx-2 transition-colors cursor-pointer">
          ...
          <span className="text-gray-500 text-xs italic">Search...</span>
        </div>
      ...
      <div className="flex items-center space-x-1 h-full border-l border-white pl-2 ml-2 shadow-[-1px_0_0_#888]">
        ...
        <div className="hover:bg-black/10 px-2 rounded-md cursor-pointer h-8 hidden md:flex items-center text-[10px] leading-tight text-center">ENG<br/>IN</div>
        <div className="hover:bg-black/10 p-1.5 rounded-md cursor-pointer flex items-center space-x-2">
          <FaWifi />
          <FaVolumeUp />
          <div className="hidden sm:flex items-center space-x-1"><FaBatteryFull className="text-green-600" /><span>100%</span></div>
        </div>
...
export default function TrainingProgram() {
  ...
  return (
    <div className="min-h-[75vh] md:min-h-screen w-full bg-[#f0ede1] flex flex-col items-center justify-end font-sans overflow-hidden pt-4 md:pt-0">
      ...
      <div className="monitor-casing-3d relative w-[95%] max-w-[1000px] bg-[#d3cebe] rounded-[2rem] p-3 md:p-5 lg:p-6 pb-10 md:pb-12 shadow-[0_40px_80px_rgba(0,0,0,0.4),inset_0_5px_20px_rgba(255,255,255,0.9),inset_0_-10px_30px_rgba(0,0,0,0.1)] z-10 mx-auto mt-2 md:mt-4">
        ...
        <div className="monitor-bezel-3d bg-[#e6e2d3] rounded-[1.5rem] p-4 md:p-8 shadow-[inset_0_15px_30px_rgba(0,0,0,0.2),0_2px_5px_rgba(255,255,255,0.5)] relative border-b-2 border-white/50">
          ...
          {/* CRT Screen */}
          <div className="crt-container w-full h-[40vh] md:h-[45vh] xl:h-[48vh] rounded-2xl overflow-hidden relative border-[8px] md:border-[10px] border-[#b8b3a3] transition-colors duration-500" style={{ backgroundColor: isScreenOn ? '#f0ece1' : '#080808' }}>
      ...
      {/* Monitor Stand */}
      <div className="flex flex-col items-center relative z-0 -mt-6 md:-mt-8 mb-0 md:mb-[-10px]">
        {/* The Neck */}
        <div className="w-24 md:w-32 h-8 md:h-10 bg-gradient-to-b from-[#8c887d] to-[#b8b3a3] shadow-[inset_0_0_20px_rgba(0,0,0,0.9)] border-x-[6px] border-[#7a766c]"></div>
        {/* The Base Plate */}
        <div className="w-56 md:w-80 h-5 md:h-6 bg-[#d3cebe] rounded-t-2xl shadow-[0_20px_40px_rgba(0,0,0,0.9),inset_0_4px_10px_rgba(255,255,255,0.9)] border-t-[2px] border-[#eae6db] flex justify-center pt-1 md:pt-2 relative">
      ...
      <div className="w-full h-[15vh] lg:h-[18vh] bg-[#d8cfc0] relative flex justify-center shadow-[inset_0_20px_50px_rgba(0,0,0,0.2)] border-t border-white/40" style={{
