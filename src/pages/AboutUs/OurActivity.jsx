// src/pages/AboutUs/OurActivity.jsx
...
/* -------------------------- HERO: SVG Overlays -------------------------- */
const ActivityHeroOverlay = () => {
  return (
    <g transform="translate(680, 50)">
      <defs>
        <pattern id="actGrid" width="30" height="30" patternUnits="userSpaceOnUse">
...
const VideoHeroActivity = ({ src = "" }) => {
  const shouldReduce = useReducedMotion();
  const title = "CRCCF Activity Hero";
  ...
  return (
    <svg viewBox="0 0 1000 400" className="w-full h-auto block" role="img" aria-label={title}>
      ...
      <rect width="100%" height="100%" fill={color.slate900} rx="20" />
      <foreignObject x="0" y="0" width="1000" height="400" mask="url(#actHeroMask)">
        <video
          src={src}
...
const getSvgComponent = (item) => {
  ...
  if (text.includes("empower") || text.includes("protection") || text.includes("shield") || text.includes("safety")) return SVG_Shield;
  ...
  if (text.includes("security") || text.includes("lock") || text.includes("privacy") || text.includes("data") || text.includes("forensics")) return SVG_Lock;
  if (text.includes("skill") || text.includes("development") || text.includes("growth") || text.includes("pipeline")) return SVG_Trending;
  if (text.includes("campaign") || text.includes("voice") || text.includes("outreach") || text.includes("public")) return SVG_Message;
  return SVG_Target;
};
      ...
      >
        <motion.nav variants={itemUp} className="mb-12">
          <button
            ...
            className="inline-flex items-center gap-2 text-emerald-600 hover:text-emerald-700 transition-colors font-bold text-sm bg-transparent border-none cursor-pointer p-0"
          >
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          ...
          <div className="py-6">
            <InsightCard allPages={activitydata} />
          </div>
        </div>
        ...
        <motion.div variants={itemUp} className="mt-16 text-center">
          <div className="bg-slate-900 rounded-[40px] p-12 md:p-16 relative overflow-hidden shadow-2xl">
            <div className="absolute top-0 right-0 w-96 h-96 bg-emerald-600 blur-[150px] opacity-20" />
            <div className="absolute bottom-0 left-0 w-64 h-64 bg-teal-600 blur-[100px] opacity-10" />
            <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 relative z-10">Want to join an activity?</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10">We are always looking for volunteers, experts, and partners to help us scale our initiatives and reach more citizens.</p>
            <div className="flex flex-wrap justify-center gap-6 relative z-10">
              <Link to="/contact" className="px-10 py-5 bg-emerald-600 text-white rounded-2xl font-bold hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-900/30 flex items-center gap-2">
