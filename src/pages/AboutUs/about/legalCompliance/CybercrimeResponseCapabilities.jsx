...
import { ArrowLeft, ArrowRight, ShieldCheck, Zap, Heart, Microscope, Users, ShieldAlert, Star, Globe, Lock, Search } from "lucide-react";
import {
  motion,
  useReducedMotion,
  useScroll,
...
// Import data
import { cybercrimeResponseCapabilitiesData } from '../../../../data/aboutUs/legalCompliance/cybercrimeResponseCapabilitiesData';
...
const getSvgIcon = (id) => {
    ...
    if (id.includes('rapid-response')) return Zap;
    if (id.includes('victim-support')) return Heart;
    if (id.includes('investigation')) return Microscope;
    if (id.includes('collaboration')) return Users;
    if (id.includes('preventive')) return ShieldAlert;
    ...
    <div className="flex flex-col gap-6 w-full max-w-2xl mx-auto">
      ...
      >
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] font-bold text-slate-400 mt-1">Response Chapter {pageIdx + 1} of {allPages.length}</span>
                </div>
              </div>
        ...
        >
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">Offensive Defense & Victim Assistance</p>

          <div className="flex items-center gap-3 text-[13px] font-black uppercase tracking-[0.15em] mt-auto text-blue-600">
            <span>Verify Capabilities</span>
...
/* ------------------------------ Main Page ------------------------------ */
export default function CybercrimeResponseCapabilities() {
      ...
      >
        ...
        <div ref={heroRef} className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
          <motion.div variants={itemUp}>
            ...
            <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-[1.05] mb-8 tracking-tight">
              Response <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Capabilities</span>
            </h1>
            <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
              Equipped to mitigate threats, recover assets, and support victims with rapid, forensic-grade precision across India.
            </p>
          </motion.div>
          ...
          <motion.div style={{ y: heroY }} className="relative flex justify-center">
            ...
            <div className="relative w-full max-w-md aspect-square bg-white rounded-[40px] shadow-2xl p-10 flex flex-col items-center justify-center border border-slate-100">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-2">Rapid Mitigation</h3>
                <p className="text-slate-500 text-center font-medium">Deploying advanced threat hunters and forensic analysts within minutes of detection.</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          <div className="mb-16 text-center">
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">The Response <span className="text-blue-600">Codex</span></h2>
            <p className="text-slate-500 text-lg font-medium">Flip through the 7 chapters of our defensive, assistive, and investigative capabilities.</p>
          </div>

          <div className="py-6">
            <InsightCard allPages={cybercrimeResponseCapabilitiesData} />
          </div>
        </div>
        ...
        <motion.div variants={itemUp} className="mt-24 grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
                { icon: Zap, label: "Real-time Action", sub: "Threat Neutralization" },
                { icon: Heart, label: "Victim Care", sub: "Recovery & Support" },
        ...
        <motion.div variants={itemUp} className="mt-24 text-center">
          <div className="bg-slate-900 rounded-[40px] p-12 md:p-16 relative overflow-hidden shadow-2xl">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 relative z-10">Restoring Digital Trust</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10">Our response capabilities are the shield that protects Bharat's digital evolution, ensuring safety and justice for all.</p>
            <div className="flex flex-wrap justify-center gap-6 relative z-10">
              <Link to="/contact" className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-900/30 flex items-center gap-2">
                Report an Incident <ArrowRight size={22} />
              </Link>
            </div>
          </div>
        </motion.div>
