...
import { ArrowLeft, ArrowRight, ShieldCheck, Cpu, Users, Globe, Scale, Microscope, Database, Lock, Award, HeartHandshake } from "lucide-react";
import {
  motion,
  useReducedMotion,
  useScroll,
...
// Import data
import { cyberInvestigationCapacityData } from '../../../../data/aboutUs/legalCompliance/cyberInvestigationCapacityData';
...
/* -------------------------- SVG Mapping -------------------------- */
const getSvgIcon = (id) => {
    if (id.includes('empowering')) return Globe;
    if (id.includes('experts')) return Users;
    if (id.includes('forensic')) return Microscope;
    if (id.includes('reach')) return Globe;
      ...
      >
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] font-bold text-slate-400 mt-1">Capacity Pillar {pageIdx + 1} of {allPages.length}</span>
                </div>
              </div>
        ...
        >
          ...
          <h3 className="text-slate-900 leading-[1.1] mb-8 text-[24px] sm:text-[32px] font-bold" style={{ fontFamily: "'Dancing Script', cursive" }}>
            The Capacity Blueprint
          </h3>
          
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">Investigative Strength & Technological Reach</p>

          <div className="flex items-center gap-3 text-[13px] font-black uppercase tracking-[0.15em] mt-auto text-blue-600">
            <span>Discover Capacity</span>
...
/* ------------------------------ Main Page ------------------------------ */
export default function OurCyberInvestigationCapacity() {
      ...
      >
        ...
        <div ref={heroRef} className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
          <motion.div variants={itemUp}>
            ...
            <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-[1.05] mb-8 tracking-tight">
              Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Investigation</span> Capacity
            </h1>
            <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
              From metropolitan hubs to rural hearts, CRCCF's capacity spans the nation, powered by advanced forensics and a mission-driven team of experts.
            </p>
          </motion.div>
          ...
          <motion.div style={{ y: heroY }} className="relative flex justify-center">
            ...
            <div className="relative w-full max-w-md aspect-square bg-white rounded-[40px] shadow-2xl p-10 flex flex-col items-center justify-center border border-slate-100">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-2">Technical Prowess</h3>
                <p className="text-slate-500 text-center font-medium">Equipped with AI-assisted forensic tools and ISO-certified secure data environments.</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          <div className="mb-16 text-center">
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">The Capacity <span className="text-blue-600">Blueprint</span></h2>
            <p className="text-slate-500 text-lg font-medium">Flip through the 5 pillars that define our strength as a national cyber investigation partner.</p>
          </div>

          <div className="py-6">
            <InsightCard allPages={cyberInvestigationCapacityData} />
          </div>
        </div>
        ...
        <motion.div variants={itemUp} className="mt-24 grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
                { icon: Database, label: "Evidence Preservation", sub: "Secure Vaults" },
                { icon: Cpu, label: "AI Forensics", sub: "Pattern Analysis" },
                { icon: Lock, label: "Data Integrity", sub: "ISO Certified" },
                { icon: Award, label: "Expert Witness", sub: "Legal Support" },
            ].map((item, idx) => (
        ...
        <motion.div variants={itemUp} className="mt-24 text-center">
          <div className="bg-slate-900 rounded-[40px] p-12 md:p-16 relative overflow-hidden shadow-2xl">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 relative z-10">Protecting Every Digital Citizen</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10">Our nationwide reach ensures that justice and technical support are accessible to every metropolitan hub and rural community.</p>
