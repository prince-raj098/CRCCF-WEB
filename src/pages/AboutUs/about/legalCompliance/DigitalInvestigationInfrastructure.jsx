...
import { ArrowLeft, ArrowRight, ShieldCheck, Database, Cpu, Network, Scroll, Microscope, Server, Lock, Globe, Landmark } from "lucide-react";
import {
  motion,
  useReducedMotion,
  useScroll,
...
// Import data
import { digitalInvestigationInfrastructureData } from '../../../../data/aboutUs/legalCompliance/digitalInvestigationInfrastructureData';
...
/* -------------------------- SVG Mapping -------------------------- */
const getSvgIcon = (id) => {
    if (id.includes('secure-foundation')) return ShieldCheck;
    if (id.includes('forensic-labs')) return Microscope;
    if (id.includes('integration-of-ai')) return Cpu;
    if (id.includes('collaborative-networks')) return Network;
    if (id.includes('ethical-governance')) return Landmark;
    return Server;
};
    ...
    <div className="flex flex-col gap-6 w-full max-w-2xl mx-auto">
      ...
      >
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] font-bold text-slate-400 mt-1">Infrastructure Component {pageIdx + 1} of {allPages.length}</span>
                </div>
              </div>
        ...
        >
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">Forensic Infrastructure & Secure Systems</p>
      ...
      >
        ...
        <div ref={heroRef} className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
          <motion.div variants={itemUp}>
            ...
            <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-[1.05] mb-8 tracking-tight">
              Investigation <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Infrastructure</span>
            </h1>
            <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
              Upholding the integrity of digital justice through a robust network of state-of-the-art forensic labs, encrypted data centers, and AI-driven monitoring.
            </p>
          </motion.div>
          ...
          <motion.div style={{ y: heroY }} className="relative flex justify-center">
            ...
            <div className="relative w-full max-w-md aspect-square bg-white rounded-[40px] shadow-2xl p-10 flex flex-col items-center justify-center border border-slate-100">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-2">Digital Citadel</h3>
                <p className="text-slate-500 text-center font-medium">Built on ISO 27001 standards for absolute information security and data privacy.</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          <div className="mb-16 text-center">
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">The Digital <span className="text-blue-600">Citadel</span></h2>
            <p className="text-slate-500 text-lg font-medium">Flip through the 5 architectural layers of our secure investigation ecosystem.</p>
          </div>

          <div className="py-6">
            <InsightCard allPages={digitalInvestigationInfrastructureData} />
          </div>
        </div>
        ...
        <motion.div variants={itemUp} className="mt-24 text-center">
          <div className="bg-slate-900 rounded-[40px] p-12 md:p-16 relative overflow-hidden shadow-2xl">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 relative z-10">Infrastructure for a Safer India</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10">Our digital citadel is more than just hardware; it is a promise of integrity, security, and lawful action in every case.</p>
            <div className="flex flex-wrap justify-center gap-6 relative z-10">
              <Link to="/contact" className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-900/30 flex items-center gap-2">
                Learn More About Our Tech <ArrowRight size={22} />
              </Link>
            </div>
          </div>
        </motion.div>
