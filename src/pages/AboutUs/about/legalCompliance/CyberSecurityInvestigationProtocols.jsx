...
import { ArrowLeft, ArrowRight, ShieldCheck, Lock, Globe, Scroll, Microscope, AlertTriangle, Eye, ShieldAlert, FileSearch, Scale } from "lucide-react";
import {
  motion,
  useReducedMotion,
  useScroll,
...
// Import data
import { cyberSecurityAndInvestigationProtocolsData } from '../../../../data/aboutUs/legalCompliance/cyberSecurityAndInvestigationProtocolsData';
...
/* -------------------------- SVG Mapping -------------------------- */
const getSvgIcon = (id) => {
    if (id.includes('commitment')) return Globe;
    if (id.includes('defense')) return ShieldCheck;
    if (id.includes('integrity')) return Scroll;
    if (id.includes('forensic')) return Microscope;
    if (id.includes('incident')) return ShieldAlert;
    if (id.includes('privacy')) return Lock;
    if (id.includes('monitoring')) return Eye;
    if (id.includes('social')) return Scale;
    return ShieldCheck;
};
    ...
    <div className="flex flex-col gap-6 w-full max-w-2xl mx-auto">
      ...
      >
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] font-bold text-slate-400 mt-1">Protocol Pillar {pageIdx + 1} of {allPages.length}</span>
                </div>
              </div>
        ...
        >
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">Defensive Protocols & Investigation Standards</p>
...
/* ------------------------------ Main Page ------------------------------ */
export default function CyberSecurityInvestigationProtocols() {
      ...
      >
        ...
        <div ref={heroRef} className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
          <motion.div variants={itemUp}>
            ...
            <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-[1.05] mb-8 tracking-tight">
              Security <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Protocols</span>
            </h1>
            <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
              Upholding national cyber resilience through standardized investigation protocols, rapid incident response, and forensic integrity.
            </p>
          </motion.div>
          ...
          <motion.div style={{ y: heroY }} className="relative flex justify-center">
            ...
            <div className="relative w-full max-w-md aspect-square bg-white rounded-[40px] shadow-2xl p-10 flex flex-col items-center justify-center border border-slate-100">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-2">Security Codex</h3>
                <p className="text-slate-500 text-center font-medium">Standardizing cyber defense and forensic imaging across all digital initiatives.</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          <div className="mb-16 text-center">
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">The Security <span className="text-blue-600">Codex</span></h2>
            <p className="text-slate-500 text-lg font-medium">Flip through the 8 pillars of our cyber security and investigation protocol framework.</p>
          </div>

          <div className="py-6">
            <InsightCard allPages={cyberSecurityAndInvestigationProtocolsData} />
          </div>
        </div>
        ...
        <motion.div variants={itemUp} className="mt-24 grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
                { icon: Lock, label: "Data Sovereignty", sub: "Privacy Focused" },
                { icon: ShieldAlert, label: "Rapid Response", sub: "Incident Mitigation" },
                { icon: FileSearch, label: "Traceability", sub: "Chain of Custody" },
                { icon: Globe, label: "National Reach", sub: "Unified Defense" },
            ].map((item, idx) => (
        ...
        <motion.div variants={itemUp} className="mt-24 text-center">
          <div className="bg-slate-900 rounded-[40px] p-12 md:p-16 relative overflow-hidden shadow-2xl">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 relative z-10">Standardizing Digital Justice</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10">Our protocols ensure that every investigation is a step toward a safer India, built on trust, precision, and the rule of law.</p>
            <div className="flex flex-wrap justify-center gap-6 relative z-10">
              <Link to="/contact" className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-900/30 flex items-center gap-2">
                Consult Our Standards <ArrowRight size={22} />
              </Link>
            </div>
          </div>
        </motion.div>
