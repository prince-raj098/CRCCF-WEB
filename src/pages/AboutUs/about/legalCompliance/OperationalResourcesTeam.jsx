...
/* -------------------------- SVG Mapping -------------------------- */
const getSvgIcon = (id) => {
    if (id.includes('strength')) return Landmark;
    if (id.includes('tools')) return Cpu;
    if (id.includes('team')) return Users;
    if (id.includes('network')) return Network;
    if (id.includes('allocation')) return BarChart3;
    if (id.includes('upgradation')) return Rocket;
    return ShieldCheck;
};
    ...
    <div className="flex flex-col gap-6 w-full max-w-2xl mx-auto">
      ...
      >
        ...
        >
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">Operational Resources & Expert Personnel</p>
...
/* ------------------------------ Main Page ------------------------------ */
export default function OperationalResourcesTeam() {
      ...
      >
        ...
        <div ref={heroRef} className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
          <motion.div variants={itemUp}>
            ...
            <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-[1.05] mb-8 tracking-tight">
              Resources & <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Team</span>
            </h1>
            <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
              Our strength lies in the strategic deployment of advanced technical resources and a dedicated team of cyber forensic experts.
            </p>
          </motion.div>
          ...
          <motion.div style={{ y: heroY }} className="relative flex justify-center">
            ...
            <div className="relative w-full max-w-md aspect-square bg-white rounded-[40px] shadow-2xl p-10 flex flex-col items-center justify-center border border-slate-100">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-2">Operational Hub</h3>
                <p className="text-slate-500 text-center font-medium">Coordinating advanced AI tools and expert investigators for national cyber defense.</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          <div className="mb-16 text-center">
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">The Arsenal of <span className="text-blue-600">Justice</span></h2>
            <p className="text-slate-500 text-lg font-medium">Flip through the 6 chapters of our operational capabilities and personnel framework.</p>
          </div>

          <div className="py-6">
            <InsightCard allPages={operationalResourcesAndTeamData} />
          </div>
        </div>
        ...
        <motion.div variants={itemUp} className="mt-24 grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
                { icon: Microscope, label: "Forensic Labs", sub: "Secure Evidence" },
                { icon: Users, label: "Expert Personnel", sub: "Certified Team" },
                { icon: GraduationCap, label: "Continuous Training", sub: "Skill Excellence" },
                { icon: Network, label: "Unified Network", sub: "Stakeholder Unity" },
            ].map((item, idx) => (
        ...
        <motion.div variants={itemUp} className="mt-24 text-center">
          <div className="bg-slate-900 rounded-[40px] p-12 md:p-16 relative overflow-hidden shadow-2xl">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 relative z-10">Powering Cyber Resilience</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10">Our operational strength is the foundation of a safer Bharat, ensuring that every digital life is protected by expert hands and advanced technology.</p>
            <div className="flex flex-wrap justify-center gap-6 relative z-10">
              <Link to="/contact" className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-900/30 flex items-center gap-2">
                Join the Expert Network <ArrowRight size={22} />
              </Link>
            </div>
          </div>
        </motion.div>
