...
import { ArrowLeft, ArrowRight, ShieldCheck, Scale, Microscope, Users, FileText, Gavel, Landmark, ShieldAlert, Star, Globe } from "lucide-react";
import {
  motion,
  useReducedMotion,
  useScroll,
...
// Import data
import { complianceWithCybercrimeRegulationData } from '../../../../data/aboutUs/legalCompliance/complianceWithCybercrimeRegulationData';
...
/* -------------------------- SVG Mapping -------------------------- */
const getSvgIcon = (id) => {
    if (id.includes('introduction')) return ShieldAlert;
    if (id.includes('framework')) return Landmark;
    if (id.includes('fairness')) return Scale;
    if (id.includes('judicial')) return Gavel;
    if (id.includes('coordination')) return Users;
    if (id.includes('evidence')) return Microscope;
    if (id.includes('conduct')) return Star;
    return ShieldCheck;
};
      ...
      >
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] font-bold text-slate-400 mt-1">Regulation Chapter {pageIdx + 1} of {allPages.length}</span>
                </div>
              </div>
        ...
        >
          ...
          <h3 className="text-slate-900 leading-[1.1] mb-8 text-[24px] sm:text-[32px] font-bold" style={{ fontFamily: "'Dancing Script', cursive" }}>
            The Regulation Codex
          </h3>
          
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">Procedural Fairness & Judicial Adherence</p>

          <div className="flex items-center gap-3 text-[13px] font-black uppercase tracking-[0.15em] mt-auto text-blue-600">
            <span>Verify Regulation</span>
...
/* ------------------------------ Main Page ------------------------------ */
export default function ComplianceWithCybercrimeRegulation() {
      ...
      >
        ...
        <div ref={heroRef} className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
          <motion.div variants={itemUp}>
            ...
            <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-[1.05] mb-8 tracking-tight">
              Cybercrime <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Regulation</span>
            </h1>
            <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
              Strictly following investigation protocols and judicial standards to ensure every forensic action is legally admissible and ethically sound.
            </p>
          </motion.div>
          ...
          <motion.div style={{ y: heroY }} className="relative flex justify-center">
            ...
            <div className="relative w-full max-w-md aspect-square bg-white rounded-[40px] shadow-2xl p-10 flex flex-col items-center justify-center border border-slate-100">
                <div className="w-24 h-24 bg-blue-50 rounded-3xl flex items-center justify-center mb-6">
                    <Gavel size={48} className="text-blue-600" />
                </div>
                <h3 className="text-2xl font-black text-slate-800 mb-2">Judicial Standards</h3>
                <p className="text-slate-500 text-center font-medium">Aligning investigations with the highest standards of the Indian judicial system.</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          <div className="mb-16 text-center">
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">The Regulation <span className="text-blue-600">Codex</span></h2>
            <p className="text-slate-500 text-lg font-medium">Flip through the 7 chapters of our adherence to cybercrime-specific regulations and protocols.</p>
          </div>

          <div className="py-6">
            <InsightCard allPages={complianceWithCybercrimeRegulationData} />
          </div>
        </div>

        {/* Regulation Highlights Grid */}
        <motion.div variants={itemUp} className="mt-24 grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
                { icon: Landmark, label: "Lawful Framework", sub: "Investigation Mandates" },
                { icon: Scale, label: "Procedural Fairness", sub: "Rights Protection" },
                { icon: Microscope, label: "Forensic Imaging", sub: "Evidence Integrity" },
                { icon: Users, label: "Agency Support", sub: "LEA Coordination" },
            ].map((item, idx) => (
        ...
        <motion.div variants={itemUp} className="mt-24 text-center">
          <div className="bg-slate-900 rounded-[40px] p-12 md:p-16 relative overflow-hidden shadow-2xl">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 relative z-10">Upholding Procedural Justice</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10">Our adherence to cybercrime regulation is the shield that ensures every victim's journey toward justice is protected by law.</p>
            <div className="flex flex-wrap justify-center gap-6 relative z-10">
              <Link to="/contact" className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-900/30 flex items-center gap-2">
                Report Investigative Need <ArrowRight size={22} />
              </Link>
            </div>
          </div>
        </motion.div>
