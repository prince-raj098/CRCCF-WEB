...
import { ArrowLeft, ArrowRight, ShieldCheck, Users, Cpu, Microscope, Settings, Network, Rocket, Landmark, Database, Terminal } from "lucide-react";
import {
  motion,
  useReducedMotion,
  useScroll,
...
/* -------------------------- SVG Mapping -------------------------- */
const getSvgIcon = (id) => {
    if (id.includes('introduction')) return Network;
    if (id.includes('professionals')) return Users;
    if (id.includes('tools')) return Terminal;
    if (id.includes('infrastructure')) return Landmark;
    if (id.includes('synergy')) return Cpu;
    if (id.includes('innovation')) return Rocket;
    return ShieldCheck;
};
    ...
    <div className="flex flex-col gap-6 w-full max-w-2xl mx-auto">
      ...
      >
        ...
        >
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">Technical Infrastructure & Expert Human Capital</p>
...
/* ------------------------------ Main Page ------------------------------ */
export default function TeamToolTechCapacity() {
      ...
      >
        ...
        <div ref={heroRef} className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
          <motion.div variants={itemUp}>
            ...
            <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-[1.05] mb-8 tracking-tight">
              Team, Tools & <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Capacity</span>
            </h1>
            <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
              Integrating human expertise with state-of-the-art forensic infrastructure to deliver unmatched cybercrime response and mitigation.
            </p>
          </motion.div>
          ...
          <motion.div style={{ y: heroY }} className="relative flex justify-center">
            ...
            <div className="relative w-full max-w-md aspect-square bg-white rounded-[40px] shadow-2xl p-10 flex flex-col items-center justify-center border border-slate-100">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-2">Technical Depth</h3>
                <p className="text-slate-500 text-center font-medium">Operating advanced forensic labs and high-performance secure servers.</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          <div className="mb-16 text-center">
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">The Capacity <span className="text-blue-600">Codex</span></h2>
            <p className="text-slate-500 text-lg font-medium">Flip through the 6 dimensions of our expert personnel and technical arsenal.</p>
          </div>

          <div className="py-6">
            <InsightCard allPages={teamToolAndTechCapacityData} />
          </div>
        </div>
        ...
        <motion.div variants={itemUp} className="mt-24 grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
                { icon: Terminal, label: "Forensic Tools", sub: "Lawful Evidence" },
                { icon: Database, label: "Secure Data", sub: "Confidentiality" },
                { icon: Users, label: "Expert Analysts", sub: "Human Intelligence" },
                { icon: Settings, label: "Operational Hub", sub: "Unified Response" },
            ].map((item, idx) => (
        ...
        <motion.div variants={itemUp} className="mt-24 text-center">
          <div className="bg-slate-900 rounded-[40px] p-12 md:p-16 relative overflow-hidden shadow-2xl">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 relative z-10">United to Protect & Serve</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10">Our team and technology are the frontline of India's cyber defense, ensuring that every digital citizen is protected by the best resources available.</p>
            <div className="flex flex-wrap justify-center gap-6 relative z-10">
              <Link to="/contact" className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-900/30 flex items-center gap-2">
                Consult Our Technical Team <ArrowRight size={22} />
              </Link>
            </div>
          </div>
        </motion.div>
