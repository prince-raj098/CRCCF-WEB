...
import { ArrowLeft, ArrowRight, ShieldCheck, Scale, FileText, Heart, Users, Search, Rocket, Landmark, Globe, ShieldAlert } from "lucide-react";
import {
  motion,
  useReducedMotion,
  useScroll,
...
// Import data
import { cybercrimeComplianceFrameworkData } from '../../../../data/aboutUs/legalCompliance/cybercrimeComplianceFrameworkData';
...
const getSvgIcon = (id) => {
    ...
    if (id.includes('policies')) return FileText;
    if (id.includes('alignment')) return Heart;
    if (id.includes('coordination')) return Users;
    if (id.includes('monitoring')) return Search;
    if (id.includes('assurance')) return ShieldCheck;
    if (id.includes('empowering')) return Rocket;
    return ShieldAlert;
};
    ...
    <div className="flex flex-col gap-6 w-full max-w-2xl mx-auto">
      ...
      >
        ...
        >
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">Systemic Compliance & Strategic Defense</p>

          <div className="flex items-center gap-3 text-[13px] font-black uppercase tracking-[0.15em] mt-auto text-blue-600">
            <span>Verify Framework</span>
...
/* ------------------------------ Main Page ------------------------------ */
export default function CybercrimeComplianceFramework() {
      ...
      >
        ...
        <div ref={heroRef} className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
          <motion.div variants={itemUp}>
            ...
            <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
              Ensuring every cyber investigation is lawful, ethical, and socially responsible through a structured and audited systemic framework.
            </p>
          </motion.div>
          ...
          <motion.div style={{ y: heroY }} className="relative flex justify-center">
            ...
            <div className="relative w-full max-w-md aspect-square bg-white rounded-[40px] shadow-2xl p-10 flex flex-col items-center justify-center border border-slate-100">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-2">Systemic Integrity</h3>
                <p className="text-slate-500 text-center font-medium">Built on the pillars of Indian cyber law, IT Act provisions, and Section 8 mandates.</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          <div className="mb-16 text-center">
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">The Framework <span className="text-blue-600">Codex</span></h2>
            <p className="text-slate-500 text-lg font-medium">Flip through the 7 chapters of our systemic compliance and strategic defense framework.</p>
          </div>

          <div className="py-6">
            <InsightCard allPages={cybercrimeComplianceFrameworkData} />
          </div>
        </div>
        ...
        <motion.div variants={itemUp} className="mt-24 grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
                { icon: Scale, label: "Legal Alignment", sub: "Statutory Adherence" },
                { icon: Search, label: "Continuous Audit", sub: "Vigilant Oversight" },
                { icon: Users, label: "Agency Synergy", sub: "Unified Action" },
                { icon: Rocket, label: "Empowered Response", sub: "Strategic Growth" },
            ].map((item, idx) => (
        ...
        <motion.div variants={itemUp} className="mt-24 text-center">
          <div className="bg-slate-900 rounded-[40px] p-12 md:p-16 relative overflow-hidden shadow-2xl">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 relative z-10">Securing Bharat's Digital Future</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10">Our compliance framework is the shield that protects every digital citizen, ensuring that safety and justice are always within reach.</p>
            <div className="flex flex-wrap justify-center gap-6 relative z-10">
              <Link to="/contact" className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-900/30 flex items-center gap-2">
                Consult Framework Experts <ArrowRight size={22} />
              </Link>
            </div>
          </div>
        </motion.div>
