...
import { ArrowLeft, ArrowRight, ShieldCheck, Scale, FileText, Lock, Users, Search, Star, Globe, Gavel, Landmark } from "lucide-react";
import {
  motion,
  useReducedMotion,
  useScroll,
...
// Import data
import { cyberLawComplianceStandardsData } from '../../../../data/aboutUs/legalCompliance/cyberLawComplianceStandardsData';
...
const getSvgIcon = (id) => {
    ...
    if (id.includes('frameworks')) return FileText;
    if (id.includes('ethical')) return Scale;
    if (id.includes('data-handling')) return Lock;
    if (id.includes('coordination')) return Users;
    if (id.includes('monitoring')) return Search;
    if (id.includes('trust')) return Star;
    return ShieldCheck;
};
    ...
    <div className="flex flex-col gap-6 w-full max-w-2xl mx-auto">
      ...
      >
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] font-bold text-slate-400 mt-1">Standards Pillar {pageIdx + 1} of {allPages.length}</span>
                </div>
              </div>
        ...
        >
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">Statutory Excellence & Compliance Paradigms</p>
...
/* ------------------------------ Main Page ------------------------------ */
export default function CyberLawComplianceStandards() {
      ...
      >
        ...
        <div ref={heroRef} className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
          <motion.div variants={itemUp}>
            ...
            <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-[1.05] mb-8 tracking-tight">
              Compliance <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Standards</span>
            </h1>
            <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
              Upholding the highest benchmarks of cyber law and ethical conduct to ensure safety and justice for every digital citizen.
            </p>
          </motion.div>
          ...
          <motion.div style={{ y: heroY }} className="relative flex justify-center">
            ...
            <div className="relative w-full max-w-md aspect-square bg-white rounded-[40px] shadow-2xl p-10 flex flex-col items-center justify-center border border-slate-100">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-2">Lawful Excellence</h3>
                <p className="text-slate-500 text-center font-medium">Operating under strict Section 8 and IT Act mandates with professional integrity.</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          <div className="mb-16 text-center">
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">The Standards <span className="text-blue-600">Codex</span></h2>
            <p className="text-slate-500 text-lg font-medium">Flip through the 7 dimensions of our statutory excellence and compliance paradigms.</p>
          </div>

          <div className="py-6">
            <InsightCard allPages={cyberLawComplianceStandardsData} />
          </div>
        </div>

        {/* Standards Highlights Grid */}
        <motion.div variants={itemUp} className="mt-24 grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
                { icon: Scale, label: "IT Act Provisions", sub: "Legal Foundation" },
                { icon: Lock, label: "Data Integrity", sub: "Secure Handling" },
                { icon: Landmark, label: "Section 8 Mandates", sub: "Non-Profit Ethics" },
                { icon: Search, label: "Procedural Audits", sub: "Continuous Monitoring" },
            ].map((item, idx) => (
        ...
        <motion.div variants={itemUp} className="mt-24 text-center">
          <div className="bg-slate-900 rounded-[40px] p-12 md:p-16 relative overflow-hidden shadow-2xl">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 relative z-10">Committed to Statutory Excellence</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10">Our compliance standards are the bedrock of our operations, ensuring that every digital interaction is governed by law and morality.</p>
            <div className="flex flex-wrap justify-center gap-6 relative z-10">
              <Link to="/contact" className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-900/30 flex items-center gap-2">
                Consult Compliance Team <ArrowRight size={22} />
              </Link>
            </div>
          </div>
        </motion.div>
