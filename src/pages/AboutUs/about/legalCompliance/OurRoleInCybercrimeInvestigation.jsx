...
import { ArrowLeft, ArrowRight, ShieldCheck, Handshake, Users, Scale, Microscope, Globe, Landmark, Heart, Search } from "lucide-react";
import {
  motion,
  useReducedMotion,
  useScroll,
...
// Import data
import { ourRoleData } from '../../../../data/aboutUs/legalCompliance/ourRoleData';
...
const getSvgIcon = (id) => {
    ...
    if (id.includes('supporting')) return Handshake;
    if (id.includes('victim')) return Heart;
    if (id.includes('forensic')) return Microscope;
    if (id.includes('partnership')) return Users;
    ...
    <div className="flex flex-col gap-6 w-full max-w-2xl mx-auto">
      ...
      >
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] font-bold text-slate-400 mt-1">Role Pillar {pageIdx + 1} of {allPages.length}</span>
                </div>
              </div>
        ...
        >
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">Operational Roles & Public Responsibility</p>

          <div className="flex items-center gap-3 text-[13px] font-black uppercase tracking-[0.15em] mt-auto text-blue-600">
            <span>Explore Our Role</span>
      ...
      >
        ...
        <div ref={heroRef} className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
          <motion.div variants={itemUp}>
            ...
            <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-[1.05] mb-8 tracking-tight">
              Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Role</span> in Cyber Justice
            </h1>
            <p className="text-xl text-slate-600 leading-relaxed max-w-xl">
              Acting as a vital bridge between society and law enforcement, we empower victims and support investigators through a blend of technology and empathy.
            </p>
          </motion.div>
          ...
          <motion.div style={{ y: heroY }} className="relative flex justify-center">
            ...
            <div className="relative w-full max-w-md aspect-square bg-white rounded-[40px] shadow-2xl p-10 flex flex-col items-center justify-center border border-slate-100">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-2">Bridge to Justice</h3>
                <p className="text-slate-500 text-center font-medium">Upholding public safety through collaborative insight and empathetic victim support.</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          <div className="mb-16 text-center">
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">The Mission <span className="text-blue-600">Mandate</span></h2>
            <p className="text-slate-500 text-lg font-medium">Flip through the 6 pillars that define our role in the national cyber safety ecosystem.</p>
          </div>

          <div className="py-6">
            <InsightCard allPages={ourRoleData} />
          </div>
        </div>

        {/* Roles Highlights Grid */}
        <motion.div variants={itemUp} className="mt-24 grid grid-cols-2 md:grid-cols-3 gap-8">
            {[
                { icon: ShieldCheck, label: "Digital Defense", sub: "Proactive Awareness" },
                { icon: Heart, label: "Empathetic Support", sub: "Victim Restoration" },
                { icon: Search, label: "Forensic Insight", sub: "Technical Assistance" },
            ].map((item, idx) => (
