...
/* -------------------------- HERO: SVG Overlays -------------------------- */
const PurposeHeroOverlay = () => {
  return (
    <g transform="translate(680, 50)">
      <defs>
        <pattern id="purpGrid" width="30" height="30" patternUnits="userSpaceOnUse">
      ...
      <rect width="300" height="300" fill="url(#purpGrid)" />
...
const VideoHeroPurpose = ({ src = "" }) => {
  const shouldReduce = useReducedMotion();
  const title = "CRCCF Purpose Hero";
  ...
  return (
    <svg viewBox="0 0 1000 400" className="w-full h-auto block" role="img" aria-label={title}>
      <defs>
        <mask id="purpHeroMask">
          <rect width="100%" height="100%" fill="white" />
          <PurposeHeroOverlay />
        </mask>
      </defs>
      <rect width="100%" height="100%" fill={color.slate900} rx="20" />
      <foreignObject x="0" y="0" width="1000" height="400" mask="url(#purpHeroMask)">
        <video
          src={src}
...
const getSvgComponent = (item) => {
  ...
  if (text.includes("light") || text.includes("beacon") || text.includes("awareness")) return SVG_Light;
  if (text.includes("guide") || text.includes("compass") || text.includes("direction")) return SVG_Compass;
  if (text.includes("key") || text.includes("foundation") || text.includes("unlock")) return SVG_Key;
  return SVG_Star;
};
    ...
    <div className="flex flex-col gap-6 w-full max-w-2xl mx-auto">
      ...
      >
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[11px] font-bold text-slate-400 mt-1">Our Purpose {pageIdx + 1} of {allPages.length}</span>
                </div>
              </div>
        ...
        >
          ...
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-[10px] mb-8">50 Pillars of our Commitment to Justice</p>

          <div className="flex items-center gap-3 text-[13px] font-black uppercase tracking-[0.15em] mt-auto text-amber-600">
            <span>Discover our Why</span>
      ...
      >
        ...
        <div ref={heroRef} className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center mb-24">
          <motion.div variants={itemUp}>
            <div className="inline-block px-4 py-1.5 bg-amber-50 text-amber-600 rounded-full text-[10px] font-black tracking-[0.2em] mb-6">
          ...
          <motion.div style={{ y: heroY }} className="relative">
            ...
            <div className="relative rounded-[40px] border border-gray-200 bg-white p-5 shadow-2xl overflow-hidden">
              <VideoHeroPurpose src="https://cdn.coverr.co/videos/coverr-a-man-writing-in-his-notebook-4541/1080p.mp4" />
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-slate-100">
          ...
          <div className="py-6">
            <InsightCard allPages={purposedata} />
          </div>
        </div>
        ...
        <motion.div variants={itemUp} className="mt-16 text-center">
          <div className="bg-slate-900 rounded-[40px] p-12 md:p-16 relative overflow-hidden shadow-2xl">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-white mb-6 relative z-10">Purpose is our north star.</h3>
            <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto relative z-10">We are committed to a long-term journey of securing the digital lives of every citizen in India.</p>
            <div className="flex flex-wrap justify-center gap-6 relative z-10">
              <Link to="/contact" className="px-10 py-5 bg-amber-600 text-white rounded-2xl font-bold hover:bg-amber-700 transition-all shadow-xl shadow-amber-900/30 flex items-center gap-2">
                Join our Mission <ArrowRight size={22} />
              </Link>
            </div>
          </div>
        </motion.div>
